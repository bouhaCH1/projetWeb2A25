...
<div class="ww-form-section">
  <div class="ww-form-card" style="max-width:540px;">
    ...
    <form id="registerForm" action="/workwave/Controller/index.php?action=register_submit" method="POST" novalidate>
      ...
      <label>Prénom *</label>
      <input type="text" name="first_name" value="<?= htmlspecialchars($_SESSION['old']['first_name'] ?? '') ?>" class="<?= !empty($fieldErrors['first_name']) ? 'ww-input-error' : '' ?>">
      <?php if (!empty($fieldErrors['first_name'])): ?>
        <div class="ww-field-err"><?= htmlspecialchars($fieldErrors['first_name']) ?></div>
      <?php endif; ?>

      <label>Nom *</label>
      <input type="text" name="last_name" value="<?= htmlspecialchars($_SESSION['old']['last_name'] ?? '') ?>" class="<?= !empty($fieldErrors['last_name']) ? 'ww-input-error' : '' ?>">
      <?php if (!empty($fieldErrors['last_name'])): ?>
        <div class="ww-field-err"><?= htmlspecialchars($fieldErrors['last_name']) ?></div>
      <?php endif; ?>

      <label>Adresse E-mail *</label>
      <input type="text" id="email" name="email" value="<?= htmlspecialchars($_SESSION['old']['email'] ?? '') ?>" class="<?= !empty($fieldErrors['email']) ? 'ww-input-error' : '' ?>">
      <?php if (!empty($fieldErrors['email'])): ?>
        <div class="ww-field-err"><?= htmlspecialchars($fieldErrors['email']) ?></div>
      <?php endif; ?>

      <label>Numéro de téléphone</label>
      <input type="text" id="phone" name="phone" value="<?= htmlspecialchars($_SESSION['old']['phone'] ?? '') ?>" class="<?= !empty($fieldErrors['phone']) ? 'ww-input-error' : '' ?>">
      <?php if (!empty($fieldErrors['phone'])): ?>
        <div class="ww-field-err"><?= htmlspecialchars($fieldErrors['phone']) ?></div>
      <?php endif; ?>

      <label>Je suis un(e) *</label>
      <select id="role" name="role" class="<?= !empty($fieldErrors['role']) ? 'ww-input-error' : '' ?>">
      ...
      <input type="password" id="confirm_password" name="confirm_password" class="<?= !empty($fieldErrors['confirm_password']) ? 'ww-input-error' : '' ?>">
