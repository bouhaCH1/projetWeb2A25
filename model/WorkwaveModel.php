...
final class WorkwaveModel
{
    ...
    public function freelancers(array $filters = []): array
    {
        ...
        $sql = 'SELECT u.email,p.* FROM users u JOIN freelancer_profiles p ON p.user_id=u.id WHERE ' . implode(' AND ', $where) . ' ORDER BY p.profile_views DESC LIMIT 100';
        return $this->rows($sql, $args);
    }
